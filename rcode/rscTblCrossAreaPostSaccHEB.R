                       ...
                       verts)
{
  ...
  if (sigFlag == TRUE & dim(temp)[1] > 0) {
    temp <- temp[temp$signifRsc == 1, ]
    a <- 0.8
    
  } else{
    temp <- temp[temp$signifRsc == 0, ]
    a <- 0.2
...
spkCorr<-read_csv('rscTblCrossAreaPostSaccade.csv',col_types = cols())

# colnames(spkCorr)
# [1] "X_unitNum"      "Y_unitNum"      "X_area"         "Y_area"         "X_visMovType"   "X_sortVisMov"   "X_errGrade"    
# [8] "X_rewGrade"     "X_poorIso"      "Y_visMovType"   "Y_sortVisMov"   "Y_errGrade"     "Y_rewGrade"     "Y_poorIso"     
# [15] "outcome"        "satCondition"   "epoch"          "rscSign"        "signifPlusRho"  "signifMinusRho" "signifRsc"     
# convert unitNums, sessNum to character type
numericCols <- c("X_unitNum","Y_unitNum")
...
# allEdges$visMovType <- already have this
...
# allEdges$sortVisMov <- already have this
# <chr>   <chr>      
# 1 #00BFC4 Vis        
# 2 #F8766D Mov        
# 3 #C77CFF VisMov     
...
sortedEdges <- sortedEdges %>% arrange(sortArea,sortVisMov,errorRewardTxt)
