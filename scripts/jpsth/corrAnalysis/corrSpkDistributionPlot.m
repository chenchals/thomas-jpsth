     ...
     for co = 1:numel(plotColConds)
         ...
         for ro = 1:numel(plotRowConds)
          ...
          for ar = 1:numel(plotPairAreas)
              ...
              roundToMs = 200;
              currDat.XY_DistBinned = zeros(size(currDat,1),1);
              nonZeroIdx = currDat.XY_Dist > 0;
              currDat.XY_DistBinned(nonZeroIdx) = round(currDat.XY_Dist(nonZeroIdx)*1000/roundToMs).*(roundToMs/1000);
